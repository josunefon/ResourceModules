...
function Invoke-StorageAccountDataAdd {
    ...
    foreach ($row in $TableRows) {
        if (($row.hash).Length -eq 0) {
            continue
        }

        if ($row.scopeName) {
            $scopeFullName = '{0}-{1}' -f $Scope, $row.scopeName
        } else {
            $scopeFullName = $Scope
        }
        New-StorageAccountTableRow -Table $TableObject -PartitionKey $row.deploymentId -DeploymentName $row.deploymentName -Hash $row.hash -Scope $scopeFullName
    }
}
...
Write-Host "[Preparation]  - storageAccountSubscriptionId: $storageAccountSubscriptionId" -Verbose
